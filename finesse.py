...
def GET():
    try:
        ...
        response = requests.get(url='https://cc01.mhc.trk:8445/finesse/api/Users', auth=HTTPBasicAuth(username, password), params='', verify=False)
        return(print(response.text))
    except:
       print ("An error occured in the GET request")
...
def POST(url, username, password, params='', data=''):
    ...
    try:
        headers = {'Content-Type': 'application/xml'}
        print ("POST() data: %s\n" % data)
        response = requests.post(url=url, auth=HTTPBasicAuth(username, password), headers=headers, params=params, data=data, verify=False)
